...
| # | 작업 | 담당 | 예상 시간 |
|---|------|------|-----------|
| 1 | **syncEngine camelCase 수정** — 동기화 pull/push 컬럼명 통일 | 개발 | 30분 |
| 2 | **다크모드 버그 전체 수정** — `#000` 버튼 → 브랜드 컬러, 하드코딩 색상 수정 | 디자인+개발 | 1시간 |
| 3 | **NoteCard React.memo** 추가 + `any` 타입 제거 | 개발 | 20분 |
| 4 | **에러 바운더리** 컴포넌트 추가 | 개발 | 20분 |
| 5 | **메시지 수정 RLS 수정** — senderId 체크 추가 | 개발 | 10분 |
| 6 | **schema.sql 폐기 또는 정리** — add-rls.sql만 유지 | 개발 | 10분 |
| 7 | **사용 안 하는 의존성 제거** (axios 등) | 개발 | 10분 |
| 8 | **노트 작성에 카테고리 선택기** 추가 | 개발 | 30분 |

### 내일 이후 착수

| # | 작업 | 우선순위 |
|---|------|----------|
| 1 | 디자인 토큰 시스템 (ThemeProvider + 시맨틱 컬러) | P0 |
| 2 | HTML 새니타이즈 (DOMPurify) | P0 |
| 3 | 인증 토큰 SecureStore 이전 | P0 |
